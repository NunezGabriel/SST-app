...
import { FormatoFormData } from "./editFormatoModal";

interface CreateFormatoModalProps {
  open: boolean;
  onClose: () => void;
...
const CreateFormatoModal: React.FC<CreateFormatoModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onCreate(form);
    setForm({ nombre: "", tipo: "Inspección", enlace: "" });
  };
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
          <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100">
            <div>
              <h2 className="text-lg font-bold text-gray-900">Crear Formato</h2>
              <p className="text-sm text-gray-400 mt-0.5">
                Completa los datos del nuevo formato
              </p>
            </div>
            <button
              onClick={handleClose}
...
export default CreateFormatoModal;
