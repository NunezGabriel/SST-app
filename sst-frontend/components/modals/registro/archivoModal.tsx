...
const ArchivoModal = ({
  ...
  // ← El input vive FUERA del árbol del modal para evitar conflictos de clicks
  ...
  const handleDocClick = (opt: DocOption) => {
    ...
    setErrorMsg("");
    setUploadedLink("");
    // Disparar el file picker directamente, sin setTimeout
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Debug — verifica que llegamos aquí
    console.log("handleFileChange disparado", { file, selectedDoc, token: user?.token });

    if (!file || !selectedDoc || !user?.token) {
      console.warn("Faltan datos:", { file: !!file, selectedDoc: !!selectedDoc, token: !!user?.token });
      return;
    }
  ...
  return (
    <>
      {/* Input FUERA del overlay — evita que el click del overlay lo interfiera */}
      <input
        ref={fileInputRef}
        type="file"
      ...
      >
        ...
        >
          ...
          <div className="p-6">
            ...
            {uploadStatus === "uploading" && (
              <div className="flex flex-col items-center gap-3 py-8">
                ...
                <p className="text-sm font-semibold text-gray-700">
                  Subiendo <span className="text-cyan-600">{selectedDoc?.label}</span>...
                </p>
            ...
            {uploadStatus === "success" && (
              <div className="flex flex-col items-center gap-4 py-6">
                ...
                <div className="text-center">
                  <p className="text-sm font-bold text-gray-800">¡Archivo subido correctamente!</p>
                  <p className="text-xs text-gray-500 mt-1">{selectedDoc?.label} — {weekLabel}</p>
                </div>
                {uploadedLink && (
                  <a href={uploadedLink} target="_blank" rel="noopener noreferrer" className="text-xs text-cyan-600 hover:underline">
                    Ver en Drive →
                  </a>
                )}
                <div className="flex gap-3 mt-2">
                  <button onClick={resetUpload} className="px-4 py-2 text-sm font-semibold bg-cyan-50 text-cyan-600 border border-cyan-200 rounded-xl hover:bg-cyan-100 transition-colors">
                    Subir otro documento
                  </button>
                  <button onClick={handleClose} className="px-4 py-2 text-sm font-semibold bg-gray-100 text-gray-600 rounded-xl hover:bg-gray-200 transition-colors">
                    Cerrar
                  </button>
                </div>
              </div>
            ...
            {/* Idle — grid de documentos */}
            {uploadStatus === "idle" && (
              <>
                <p className="text-sm text-gray-500 mb-5">Selecciona el tipo de documento a subir:</p>
                  ...
                  {options.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className={`text-[11px] font-bold text-center leading-tight ${c.text}`}>
                          {opt.label}
                        </span>
